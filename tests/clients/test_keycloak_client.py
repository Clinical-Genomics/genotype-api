...
def test_get_client_success(mocker):
    ...
    mock_keycloak_openid = mocker.patch("genotype_api.clients.authentication.keycloak_client.KeycloakOpenID")
...
def test_get_client_connection_error(mocker):
    ...
    mocker.patch(
        "genotype_api.clients.authentication.keycloak_client.KeycloakOpenID",
        side_effect=KeycloakConnectionError("Connection failed"),
    )
    ...
    with pytest.raises(
        KeycloakConnectionError, match="Failed to connect to Keycloak: Connection failed"
    ):
        client.get_client()
...
def test_get_client_general_exception(mocker):
    ...
    with pytest.raises(
        Exception, match="An error occurred while creating Keycloak client: General error"
    ):
        client.get_client()
